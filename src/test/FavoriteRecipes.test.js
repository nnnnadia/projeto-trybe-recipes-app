...
describe('Testando página de FavoriteRecipes', () => {
  ...
  it('Verifica se ao clicar no  botão para desfavoritar o elemento some da tela', () => {
    const { history } = renderWithRouter(<App />);
    ...
    const foodName = screen.getByText(FOOD_NAME);
    const drinkName = screen.getByText(/Aquamarine/i);
    expect(foodName).toBeInTheDocument();
    expect(drinkName).toBeInTheDocument();

    const favoriteButtonFood = screen.getByTestId('0-horizontal-favorite-btn');
    userEvent.click(favoriteButtonFood);

    const foodNameAfter = screen.queryByText(FOOD_NAME);
    expect(foodNameAfter).not.toBeInTheDocument();

    const favoriteButtonDrink = screen.getByTestId('0-horizontal-favorite-btn');
    userEvent.click(favoriteButtonDrink);

    const drinkNameAfter = screen.queryByText(/Aquamarine/i);
    expect(drinkNameAfter).not.toBeInTheDocument();
  });
});
